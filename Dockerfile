...
RUN apt-get update \
    && apt-get install -y --no-install-recommends \
        ...
        unrar-free \
        unzip \
    && url="$(curl -fsS https://api.github.com/repos/morpheus65535/bazarr/releases/latest | jq -r '.assets[]|select(.name=="bazarr.zip")|.browser_download_url')" \
    && curl -fLOsS "${url:?}" \
    && mkdir -p /app \
...
EXPOSE 6767
WORKDIR /app
ENTRYPOINT ["/app/venv/bin/python3", "/app/bazarr.py", "--config=/data", "--no-update=true"]
